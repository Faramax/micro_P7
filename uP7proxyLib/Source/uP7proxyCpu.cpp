...
bool CProxyCpu::ApplyDescription(uint32_t i_uSessionId)
{
       ...
       )
    {
        ...
        if (l_bReturn)
        {
            ...
            uint64_t l_qwStartTime = MAXUINT64;

            for (size_t l_szI = 0; l_szI < uP7_CPU_STREAMS_COUNT; l_szI++)
            {
                uint64_t l_qwTime = MAXUINT64;
                if (m_cStreams[l_szI].iStream->ExtractStartTime(m_cStreams[l_szI].pFirst, l_qwTime))
                {
                    if (l_qwTime < l_qwStartTime)    
                    {
                        l_qwStartTime = l_qwTime;
                    }
                }
            }

            if (MAXUINT64 != l_qwStartTime)
            {
                for (size_t l_szI = 0; l_szI < uP7_CPU_STREAMS_COUNT; l_szI++)
                {
                    m_cStreams[l_szI].iStream->SetStartTime(l_qwStartTime);
                }
            }
            else
            {
                uERROR(TM("[CPU#%d] Can't extract time from the streams"), (int)m_bId); 
            }
